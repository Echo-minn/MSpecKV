...
def _load_rows(path: str, metric: str) -> List[Row]:
    ...
    triforce = methods.get("triforce_kv_quant", {})
    sweep = triforce.get("sweep", {})
    if not isinstance(sweep, dict) or len(sweep) == 0:
        raise ValueError(f"{path}: no methods.triforce_kv_quant.sweep found")

    x_key = _detect_x_key_from_sweep(sweep)

    rows: List[Row] = []
    # Support both JSON schemas:
    # - Old: methods['triforce_kv_quant']['sweep'][x] has nested {'baseline': {...}, 'quant': {...}}
    # - New: methods['triforce']['sweep'][x] is baseline, methods['triforce_kv_quant']['sweep'][x] is quant (flat)
    is_old_nested = False
    ...
    triforce_baseline_sweep = methods.get("triforce", {}).get("sweep", {}) if isinstance(methods, dict) else {}
    ...
    for k, entry in sweep.items():
        ...
        if is_old_nested:
            ...
            # New flat format: kv-quant entry is `entry`, baseline is stored under methods['triforce']['sweep'].
            if not isinstance(triforce_baseline_sweep, dict):
                raise ValueError(
                    f"{path}: expected methods.triforce.sweep for baseline comparison, "
                    f"but found {type(triforce_baseline_sweep).__name__}"
                )
            baseline_block = triforce_baseline_sweep.get(k, {})
            quant_block = entry
        ...
        try:
            ...
            raise KeyError(
                ...
                f"Schema hint: old format needs sweep[x].baseline.*, new format needs methods.triforce.sweep[x].*"
            ) from e
        try:
            ...
            raise KeyError(
                ...
                f"Schema hint: old format needs sweep[x].quant.*, new format needs methods.triforce_kv_quant.sweep[x].*"
            ) from e
        d = q - b
