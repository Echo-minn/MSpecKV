"""
This is context length ablation study for offloading with Quantized_MSpecKV.
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Context length (prefill) ablation study for offloading with Quantized_MSpecKV')
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='existing JSON results to load + update (e.g., reuse prior MSpecKV/MSpecKV results)'
    )
    parser.add_argument(
        '--run_methods',
        type=str,
        default='baseline,vanilla,mspeckv,kvquant',
        help='comma-separated subset of: baseline,vanilla,mspeckv,kvquant (e.g., "baseline,vanilla")'
    )
...
def _parse_run_methods(run_methods_str: str):
    allowed = {'baseline', 'vanilla', 'mspeckv', 'kvquant'}
    ...
    if len(methods_set) == 0:
        raise ValueError("--run_methods resolved to empty set; provide at least one of: baseline,vanilla,mspeckv,kvquant")
    return methods_set

run_methods = _parse_run_methods(getattr(args, 'run_methods', 'baseline,vanilla,mspeckv,kvquant'))
run_baseline = 'baseline' in run_methods
run_vanilla = 'vanilla' in run_methods
run_mspeckv = 'mspeckv' in run_methods
run_kvquant = 'kvquant' in run_methods


def _upgrade_old_mspeckv_kv_quant_format(results: dict) -> dict:
    """
    Backward-compat: older cxt_len_ablation.json stored MSpecKV baseline + KV-quant under:
      methods['mspeckv_kv_quant']['sweep'][p] = {'baseline': {...}, 'quant': {...}, ...}
    Newer format stores:
      methods['mspeckv']['sweep'][p] (flat)
      methods['mspeckv_kv_quant']['sweep'][p] (flat)
    ...
    if not isinstance(methods, dict):
        return results
    tkv = methods.get('mspeckv_kv_quant')
    ...
    methods.setdefault('mspeckv', {'name': 'MSpecKV (SpecDec + Retrieval)', 'sweep': {}})
    if not isinstance(methods['mspeckv'], dict):
        methods['mspeckv'] = {'name': 'MSpecKV (SpecDec + Retrieval)', 'sweep': {}}
    methods['mspeckv'].setdefault('sweep', {})
    if not isinstance(methods['mspeckv']['sweep'], dict):
        methods['mspeckv']['sweep'] = {}
    ...
    for p_str, entry in list(sweep.items()):
        ...
        if isinstance(quant_entry, dict) and len(quant_entry) > 0:
            # Flatten into mspeckv_kv_quant sweep entry
            sweep[p_str] = {
                'prefill_length': int(entry.get('prefill_length', int(p_str))),
                'resident_layers': int(entry.get('resident_layers', entry.get('kv_resident_max_layers', 0) or 0)) or None,
...
if getattr(args, 'input_json', None) is not None and os.path.isfile(args.input_json):
    ...
    comparison_results = _upgrade_old_mspeckv_kv_quant_format(comparison_results)
...
recent_size = None
if run_vanilla or run_mspeckv or run_kvquant:
...
# =============================================================================
# Context length (prefill) ablation study for offloading with Quantized_MSpecKV
# =============================================================================
if local_rank == 0:
    print(colored("\n" + "=" * 80, "yellow"))
    print(colored("Context length (prefill) ablation study for offloading with Quantized_MSpecKV", "yellow"))
...
comparison_results['methods'].setdefault('mspeckv', {'name': 'MSpecKV (SpecDec + Retrieval)', 'retrieval_budget': retrieval_budget, 'gamma': gamma, 'sweep': {}})
...
for _k in ['baseline', 'vanilla_specdec', 'mspeckv', 'mspeckv_kv_quant']:
...
for sweep_idx, prefill_length in enumerate(prefill_lengths_values):
    ...
    def run_mspeckv_one_setting(*, quant: bool):
        ...
        for idx, input_ids in enumerate(tqdm(
            tokenized_prompts,
            desc=f"MSpecKV_{'KVQuant' if quant else 'Baseline'}(prefill_length={prefill_length})",
            disable=(local_rank != 0)
        )):
            input_ids = input_ids[:, :prefill_length].to(llm.device)
    ...
    # ----------------------------
    # Run selected methods (can skip MSpecKV/MSpecKV and merge with existing JSON via --input_json)
    # ----------------------------
    baseline_latencies_s, baseline_latency_per_token_ms = (None, None)
    vanilla_latencies_s, vanilla_accepted_tokens = (None, None)
    mspeckv_baseline_latencies, mspeckv_baseline_latency_per_token_s, mspeckv_baseline_accepted_tokens = (None, None, None)
    mspeckv_quant_latencies, mspeckv_quant_latency_per_token_s, mspeckv_quant_accepted_tokens = (None, None, None)
    ...
    if run_mspeckv:
        mspeckv_baseline_latencies, mspeckv_baseline_latency_per_token_s, mspeckv_baseline_accepted_tokens = run_mspeckv_one_setting(quant=False)
    if run_kvquant:
        mspeckv_quant_latencies, mspeckv_quant_latency_per_token_s, mspeckv_quant_accepted_tokens = run_mspeckv_one_setting(quant=True)
    ...
    if local_rank == 0:
        ...
        t_prev = methods.get('mspeckv', {}).get('sweep', {}).get(p_str, {}) if isinstance(methods, dict) else {}
        q_prev = methods.get('mspeckv_kv_quant', {}).get('sweep', {}).get(p_str, {}) if isinstance(methods, dict) else {}
        ...
        # MSpecKV baseline stats
        if run_mspeckv and mspeckv_baseline_latencies is not None and len(mspeckv_baseline_latencies) > 0:
            mean_mspeckv_baseline_latency = float(np.mean(mspeckv_baseline_latencies))
            std_mspeckv_baseline_latency = float(np.std(mspeckv_baseline_latencies))
            mean_mspeckv_baseline_throughput = float(gen_len / mean_mspeckv_baseline_latency) if mean_mspeckv_baseline_latency > 0 else None
            mean_mspeckv_baseline_accepted = float(np.mean(mspeckv_baseline_accepted_tokens)) if mspeckv_baseline_accepted_tokens is not None and len(mspeckv_baseline_accepted_tokens) > 0 else None
            mean_mspeckv_baseline_latency_per_token_s = float(np.mean(mspeckv_baseline_latency_per_token_s)) if mspeckv_baseline_latency_per_token_s is not None and len(mspeckv_baseline_latency_per_token_s) > 0 else None
        else:
            mean_mspeckv_baseline_latency = t_prev.get('mean_latency_s')
            std_mspeckv_baseline_latency = t_prev.get('std_latency_s')
            mean_mspeckv_baseline_throughput = t_prev.get('mean_throughput_tok_per_s')
            mean_mspeckv_baseline_accepted = t_prev.get('mean_accepted_tokens')
            mean_mspeckv_baseline_latency_per_token_s = t_prev.get('mean_latency_s_per_token')

        # KV-quant stats
        if run_kvquant and mspeckv_quant_latencies is not None and len(mspeckv_quant_latencies) > 0:
            mean_mspeckv_quant_latency = float(np.mean(mspeckv_quant_latencies))
            std_mspeckv_quant_latency = float(np.std(mspeckv_quant_latencies))
            mean_mspeckv_quant_throughput = float(gen_len / mean_mspeckv_quant_latency) if mean_mspeckv_quant_latency > 0 else None
            mean_mspeckv_quant_accepted = float(np.mean(mspeckv_quant_accepted_tokens)) if mspeckv_quant_accepted_tokens is not None and len(mspeckv_quant_accepted_tokens) > 0 else None
            mean_mspeckv_quant_latency_per_token_s = float(np.mean(mspeckv_quant_latency_per_token_s)) if mspeckv_quant_latency_per_token_s is not None and len(mspeckv_quant_latency_per_token_s) > 0 else None
        else:
            mean_mspeckv_quant_latency = q_prev.get('mean_latency_s')
            std_mspeckv_quant_latency = q_prev.get('std_latency_s')
            mean_mspeckv_quant_throughput = q_prev.get('mean_throughput_tok_per_s')
            mean_mspeckv_quant_accepted = q_prev.get('mean_accepted_tokens')
            mean_mspeckv_quant_latency_per_token_s = q_prev.get('mean_latency_s_per_token')
        ...
        speedup_mspeckv_vs_baseline = (mean_baseline_latency / mean_mspeckv_baseline_latency) if (mean_baseline_latency and mean_mspeckv_baseline_latency) else None
        speedup_quant_vs_baseline = (mean_baseline_latency / mean_mspeckv_quant_latency) if (mean_baseline_latency and mean_mspeckv_quant_latency) else None
        speedup_mspeckv_vs_vanilla = (mean_vanilla_latency / mean_mspeckv_baseline_latency) if (mean_vanilla_latency and mean_mspeckv_baseline_latency) else None
        speedup_quant_vs_vanilla = (mean_vanilla_latency / mean_mspeckv_quant_latency) if (mean_vanilla_latency and mean_mspeckv_quant_latency) else None
        speedup_quant_vs_mspeckv = (mean_mspeckv_baseline_latency / mean_mspeckv_quant_latency) if (mean_mspeckv_baseline_latency and mean_mspeckv_quant_latency) else None

        # Save / merge sweep entries (do not clobber existing MSpecKV/KVQuant metrics if we skipped them)
        ...
        methods.setdefault('mspeckv', {'name': 'MSpecKV (SpecDec + Retrieval)', 'retrieval_budget': retrieval_budget, 'gamma': gamma, 'sweep': {}})
        methods.setdefault('mspeckv_kv_quant', {'name': 'MSpecKV with KV Cache Quantization', 'retrieval_budget': retrieval_budget, 'gamma': gamma, 'sweep': {}})
        for _k in ['baseline', 'vanilla_specdec', 'mspeckv', 'mspeckv_kv_quant']:
        ...
        # MSpecKV (only overwrite mean metrics if we ran it; always refresh speedups if possible)
        t_entry = methods['mspeckv']['sweep'].get(p_str, {'prefill_length': int(prefill_length), 'resident_layers': int(resident_layers)})
        if run_mspeckv:
            t_entry.update({
                'prefill_length': int(prefill_length),
                'resident_layers': int(resident_layers),
                'mean_latency_s': mean_mspeckv_baseline_latency,
                'std_latency_s': std_mspeckv_baseline_latency,
                'mean_throughput_tok_per_s': mean_mspeckv_baseline_throughput,
                'mean_accepted_tokens': mean_mspeckv_baseline_accepted,
                'mean_latency_s_per_token': mean_mspeckv_baseline_latency_per_token_s,
            })
        # Always refresh speedups based on latest baseline/vanilla (if available)
        t_entry['speedup_vs_baseline'] = speedup_mspeckv_vs_baseline
        t_entry['speedup_vs_vanilla'] = speedup_mspeckv_vs_vanilla
        methods['mspeckv']['sweep'][p_str] = t_entry

        # KVQuant (only overwrite mean metrics if we ran it; always refresh speedups if possible)
        q_entry = methods['mspeckv_kv_quant']['sweep'].get(p_str, {'prefill_length': int(prefill_length), 'resident_layers': int(resident_layers)})
        if run_kvquant:
            q_entry.update({
                'prefill_length': int(prefill_length),
                'resident_layers': int(resident_layers),
                'mean_latency_s': mean_mspeckv_quant_latency,
                'std_latency_s': std_mspeckv_quant_latency,
                'mean_throughput_tok_per_s': mean_mspeckv_quant_throughput,
                'mean_accepted_tokens': mean_mspeckv_quant_accepted,
        ...
        q_entry['speedup_vs_mspeckv'] = speedup_quant_vs_mspeckv
        methods['mspeckv_kv_quant']['sweep'][p_str] = q_entry
        ...
        print(colored(
            ...
            f"Latency: {_fmt_num(mean_mspeckv_baseline_latency, 4)} ± {_fmt_num(std_mspeckv_baseline_latency, 4)} s | "
            f"Throughput: {_fmt_num(mean_mspeckv_baseline_throughput, 2)} tok/s | "
            f"Accepted: {_fmt_num(mean_mspeckv_baseline_accepted, 2)} | "
            f"Speedup vs Baseline: {_fmt_speed(speedup_mspeckv_vs_baseline)}",
            "green"
        ))
        print(colored(
            f"[MSpecKV][p={prefill_length}] "
            f"Latency: {_fmt_num(mean_mspeckv_quant_latency, 4)} ± {_fmt_num(std_mspeckv_quant_latency, 4)} s | "
            f"Throughput: {_fmt_num(mean_mspeckv_quant_throughput, 2)} tok/s | "
            f"Accepted: {_fmt_num(mean_mspeckv_quant_accepted, 2)} | "
            f"Speedup vs Baseline: {_fmt_speed(speedup_quant_vs_baseline)} | "
            f"Speedup vs MSpecKV: {_fmt_speed(speedup_quant_vs_mspeckv)}",
            "green"
        ))

    # (models freed inside run_mspeckv_one_setting)
...
if local_rank == 0:
    ...
    if output_csv is not None:
        with open(output_csv, 'w', newline='') as f:
            ...
            writer.writerow([
                ...
                'mspeckv_mean_latency_s',
                'mspeckv_std_latency_s',
                'mspeckv_mean_throughput_tok_per_s',
                'mspeckv_mean_accepted_tokens',
                'mspeckv_mean_latency_s_per_token',
                ...
                'speedup_mspeckv_vs_baseline',
                'speedup_kvquant_vs_baseline',
                'speedup_mspeckv_vs_vanilla',
                'speedup_kvquant_vs_vanilla',
                'speedup_kvquant_vs_mspeckv',
            ])
            for prefill_length in prefill_lengths_values:
                ...
                t = comparison_results['methods']['mspeckv']['sweep'].get(str(prefill_length), {})
                q = comparison_results['methods']['mspeckv_kv_quant']['sweep'].get(str(prefill_length), {})
                writer.writerow([
                    prefill_length,
                    args.on_chip,
    ...
    print(f"\n{'prefill':>8} | {'vanilla_vs_base':>15} | {'mspeckv_vs_base':>15} | {'kvquant_vs_base':>15} | {'kvquant_vs_mspeckv':>18}")
    print("-" * 86)
    for p in prefill_lengths_values:
        v = comparison_results['methods']['vanilla_specdec']['sweep'].get(str(p), {})
        t = comparison_results['methods']['mspeckv']['sweep'].get(str(p), {})
        q = comparison_results['methods']['mspeckv_kv_quant']['sweep'].get(str(p), {})
        def fmt(x):
            return f"{x:.2f}x" if isinstance(x, (float, int)) and x is not None else "NA"
        print(f"{p:>8} | {fmt(v.get('speedup_vs_baseline')):>15} | {fmt(t.get('speedup_vs_baseline')):>15} | {fmt(q.get('speedup_vs_baseline')):>15} | {fmt(q.get('speedup_vs_mspeckv')):>18}")
