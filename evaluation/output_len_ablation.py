"""
This is output length ablation study for offloading with Quantized_TriForce.
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Output length ablation study for offloading with Quantized_TriForce')
...
if local_rank == 0:
    print(colored("\n" + "=" * 80, "yellow"))
    print(colored("Output length ablation study for offloading with Quantized_TriForce", "yellow"))
...
for sweep_idx, gen_len_sweep in enumerate(gen_lens_values):
    ...
    def run_triforce_one_setting(*, quant: bool):
        ...
        for idx, input_ids in enumerate(tqdm(
            tokenized_prompts,
            desc=f"TriForce_{'KVQuant' if quant else 'Baseline'}(gen_len={gen_len_sweep})",
    ...
    if local_rank == 0:
        ...
        comparison_results['methods']['triforce_kv_quant']['sweep'][str(gen_len_sweep)] = {
        ...
        print(colored(
            ...
            f"[MSpecKV][gen_len={gen_len_sweep}] Speedup vs TriForce Baseline: N/A",
            "green"
        ))
