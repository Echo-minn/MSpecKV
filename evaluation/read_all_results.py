...
Designed for `evaluation/cxt_len_ablation.py` outputs, including the newer schema:
  methods:
    ...
    mspeckv:         sweep[x] -> {mean_latency_s, mean_throughput_tok_per_s, mean_accepted_tokens, ...}
    mspeckv_kv_quant:sweep[x] -> {mean_latency_s, mean_throughput_tok_per_s, mean_accepted_tokens, ...}

Backward compatible with older nested format:
  methods['mspeckv_kv_quant']['sweep'][x] contains {'baseline': {...}, 'quant': {...}}
...
def _upgrade_old_schema_in_place(data: Dict[str, Any]) -> None:
    ...
    # Ensure methods['mspeckv'] exists
    mspeckv = methods.setdefault("mspeckv", {"name": "MSpecKV", "sweep": {}})
    if not isinstance(mspeckv, dict):
        methods["mspeckv"] = {"name": "MSpecKV", "sweep": {}}
        mspeckv = methods["mspeckv"]
    mspeckv.setdefault("sweep", {})
    if not isinstance(mspeckv["sweep"], dict):
        mspeckv["sweep"] = {}
...
def main() -> None:
    ...
    ap.add_argument(
        "--methods",
        type=str,
        default="baseline,vanilla_specdec,mspeckv,mspeckv_kv_quant",
