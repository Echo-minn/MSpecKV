"""
This is on-chip layer ablation study for offloading with Quantized_TriForce.
CUDA_VISIBLE_DEVICES=2 OMP_NUM_THREADS=48 torchrun --nproc_per_node=1 evaluation/on_chip_ablation.py --budget 4096 --prefill 8192 --dataset demo --target llama-7B-128K --on_chip 16 --gamma 8 --resident_layers 16 --output_json auto
"""
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='On-chip layer ablation study for offloading with Quantized_TriForce')
    ...
    if output_csv == 'auto' or output_json == 'auto' or args.separate_files:
        ...
        args.output_csv = f"B1/on_chip_ablation_{base_name}_{timestamp_str}.csv"
        args.output_json = f"B1/on_chip_ablation_{base_name}_{timestamp_str}.json"
...
# On-chip layer sweep values
try:
    on_chip_layers_values = [int(x.strip()) for x in args.on_chip_layers.split(',') if x.strip() != '']
except Exception as e:
    raise ValueError(
        f"Failed to parse --on_chip_layers='{args.on_chip_layers}'. Expected comma-separated ints."
    ) from e
if len(on_chip_layers_values) == 0:
    # If user passes empty string, fall back to single value from --on_chip (int).
    on_chip_layers_values = [args.on_chip]
...
comparison_results = {
    'config': {
        ...
        'on_chip_layers_sweep': on_chip_layers_values,
...
# =============================================================================
# On-chip layer ablation study for offloading with Quantized_TriForce
# =============================================================================
if local_rank == 0:
    print(colored("\n" + "=" * 80, "yellow"))
    print(colored("On-chip layer ablation study for offloading with Quantized_TriForce", "yellow"))
...
for sweep_idx, on_chip_layers in enumerate(on_chip_layers_values):
    if local_rank == 0:
        print(colored("\n" + "-" * 80, "cyan"))
        print(colored(f"[Sweep {sweep_idx+1}/{len(on_chip_layers_values)}] on_chip_layers={on_chip_layers}", "cyan"))
    ...
    def run_triforce_one_setting(*, quant: bool):
        ...
        for idx, input_ids in enumerate(tqdm(
            tokenized_prompts,
            desc=f"TriForce_{'KVQuant' if quant else 'Baseline'}(on_chip_layers={on_chip_layers})",
    ...
    if local_rank == 0:
        ...
        comparison_results['methods']['triforce_kv_quant']['sweep'][str(on_chip_layers)] = {
            'on_chip_layers': int(on_chip_layers),
            'resident_layers': args.resident_layers,
            'baseline': {
        ...
        print(colored(
            f"\n[TriForce][on_chip_layers={on_chip_layers}] "
            f"Mean Latency: {mean_baseline_latency:.4f} ± {std_baseline_latency:.4f} s",
            "green"
        ))
        print(colored(
            f"[TriForce][on_chip_layers={on_chip_layers}] "
        ...
        print(colored(
            f"\n[MSpecKV][on_chip_layers={on_chip_layers}] "
            f"Mean Latency: {mean_quant_latency:.4f} ± {std_quant_latency:.4f} s",
            "green"
        ))
        print(colored(
            f"[MSpecKV][on_chip_layers={on_chip_layers}] "
        ...
        print(colored(
            ...
            f"[MSpecKV][on_chip_layers={on_chip_layers}] Speedup vs TriForce Baseline: N/A",
            "green"
        ))
...
if local_rank == 0:
    ...
    if output_csv is not None:
        with open(output_csv, 'w', newline='') as f:
            ...
            for on_chip_layers in on_chip_layers_values:
                row = sweep_dict.get(str(on_chip_layers), {})
