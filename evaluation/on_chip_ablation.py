"""
This is on-chip layer ablation study for offloading with Quantized_MSpecKV.
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='On-chip layer ablation study for offloading with Quantized_MSpecKV')
...
if local_rank == 0:
    print(colored("\n" + "=" * 80, "yellow"))
    print(colored("On-chip layer ablation study for offloading with Quantized_MSpecKV", "yellow"))
...
for sweep_idx, on_chip_layers in enumerate(on_chip_layers_values):
    ...
    def run_mspeckv_one_setting(*, quant: bool):
        ...
        for idx, input_ids in enumerate(tqdm(
            tokenized_prompts,
            desc=f"MSpecKV_{'KVQuant' if quant else 'Baseline'}(on_chip_layers={on_chip_layers})",
    ...
    if local_rank == 0:
        ...
        comparison_results['methods']['mspeckv_kv_quant']['sweep'][str(on_chip_layers)] = {
        ...
        print(colored(
            ...
            f"[MSpecKV][on_chip_layers={on_chip_layers}] Speedup vs MSpecKV Baseline: N/A",
            "green"
        ))
