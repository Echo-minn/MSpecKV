...
class DistributedLlama:
    def __init__(self, 
        ...
        if kv_offload:
            assert bsz == 1
            self.kv_cache =  DistributedSimpleCache(
                self.config,
                max_budget=prefill+gen_len+tree_size,
                device=self.device,
                on_chip_layers=on_chip_layers,
                ssl=ssl,
                kv_cache_quant=kv_cache_quant,
                kv_cache_quant_bits=kv_cache_quant_bits,
                kv_fp16_tail=kv_fp16_tail,
                kv_resident_gpu=kv_resident_gpu,
                kv_resident_max_layers=kv_resident_max_layers,
            )
        ...
        self._kv_prefetch_ev = [torch.cuda.Event() for _ in range(2)]
        self._kv_compute_done_ev = [torch.cuda.Event() for _ in range(2)]
        self._kv_offload_done_ev = [torch.cuda.Event() for _ in range(2)]
    ...
    def inference(self,
        ...
        if self.kv_offload:
            ...
            first_off = self.on_chip_layers
            if first_off < self.num_layers:
                b0 = first_off % 2
                with torch.cuda.stream(self.load_stream):
                    self.kv_buffer[b0].copy_kv(self.kv_cache, first_off)
                    self._kv_prefetch_ev[b0].record(self.load_stream)
                if first_off + 1 < self.num_layers:
                    b1 = (first_off + 1) % 2
                    with torch.cuda.stream(self.load_stream):
                        self.kv_buffer[b1].copy_kv(self.kv_cache, first_off + 1)
                        self._kv_prefetch_ev[b1].record(self.load_stream)
            ...
            for idx in range(self.num_layers):
                ...
                buf = idx % 2
                cur_stream.wait_event(self._kv_prefetch_ev[buf])

                next_idx = idx + 1
                if next_idx < self.num_layers:
                    next_buf = next_idx % 2
                    with torch.cuda.stream(self.load_stream):
                        self.kv_buffer[next_buf].copy_kv(self.kv_cache, next_idx)
                        self._kv_prefetch_ev[next_buf].record(self.load_stream)

                hidden_states = self.layer_compute(self.layers[idx], idx, hidden_states, position_ids, attention_mask, retrieval_cache)
                self._kv_compute_done_ev[buf].record(cur_stream)

                with torch.cuda.stream(self.load_stream):
                    self.load_stream.wait_event(self._kv_compute_done_ev[buf])
                    self.kv_cache.copy_back_from_buffer(self.kv_buffer[buf], idx)
                    self._kv_offload_done_ev[buf].record(self.load_stream)

            if self.num_layers - 1 >= self.on_chip_layers:
                cur_stream.wait_event(self._kv_offload_done_ev[(self.num_layers - 1) % 2])
