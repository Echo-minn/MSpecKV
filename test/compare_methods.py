...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Compare: Baseline vs Vanilla SpecDec vs TriForce')
...
# =============================================================================
# METHOD 4: TriForce with KV Cache Quantization
# =============================================================================
if local_rank == 0:
    print(colored("\n" + "=" * 80, "yellow"))
...
if local_rank == 0:
    ...
    speedup_triforce = mean_baseline_latency / mean_triforce_latency
    speedup_vs_vanilla = mean_vanilla_latency / mean_triforce_latency
    ...
    comparison_results['methods']['triforce'] = {
        ...
        'speedup_vs_baseline': float(speedup_triforce),
    ...
    print(colored(f"[TriForce] Speedup vs Baseline: {speedup_triforce:.2f}x", "green"))
    print(colored(f"[TriForce] Speedup vs Vanilla SpecDec: {speedup_vs_vanilla:.2f}x", "green"))
...
if local_rank == 0:
    speedup_triforce = mean_baseline_latency / mean_triforce_latency
    speedup_vs_vanilla = mean_vanilla_latency / mean_triforce_latency
    speedup_vs_baseline_quant = mean_baseline_latency / mean_triforce_quant_latency
    speedup_vs_vanilla_quant = mean_vanilla_latency / mean_triforce_quant_latency
    speedup_vs_triforce_quant = mean_triforce_latency / mean_triforce_quant_latency

    comparison_results['methods']['triforce']['speedup_vs_baseline'] = float(speedup_triforce)
    comparison_results['methods']['triforce']['speedup_vs_vanilla'] = float(speedup_vs_vanilla)
    comparison_results['methods']['triforce_kv_quant']['speedup_vs_baseline'] = float(speedup_vs_baseline_quant)
    comparison_results['methods']['triforce_kv_quant']['speedup_vs_vanilla'] = float(speedup_vs_vanilla_quant)
    comparison_results['methods']['triforce_kv_quant']['speedup_vs_triforce'] = float(speedup_vs_triforce_quant)
...
if local_rank == 0:
    ...
    print(f"{'TriForce (SpecDec + Retrieval)':<35} {mean_triforce_latency:>6.4f} ± {std_triforce_latency:<5.4f} {mean_triforce_throughput:>8.2f} tok/s  {speedup_triforce:<10.2f}x")
    print(f"{'TriForce (KV Cache Quantization)':<35} {mean_triforce_quant_latency:>6.4f} ± {std_triforce_quant_latency:<5.4f} {mean_triforce_quant_throughput:>8.2f} tok/s  {speedup_vs_baseline_quant:<10.2f}x {speedup_vs_triforce_quant:<10.2f}x")
    ...
    with open(csv_file, 'a' if file_exists else 'w', newline='') as f:
        ...
        if not file_exists:
            writer.writerow([
                ...
                'triforce_latency_s', 'triforce_throughput', 'triforce_accepted_tokens', 'triforce_speedup_vs_baseline', 'triforce_speedup_vs_vanilla',
                'triforce_quant_latency_s', 'triforce_quant_throughput', 'triforce_quant_accepted_tokens', 'triforce_quant_speedup_vs_baseline', 'triforce_quant_speedup_vs_vanilla', 'triforce_quant_speedup_vs_triforce'
            ])
        ...
        writer.writerow([
            ...
            f"{speedup_vs_triforce_quant:.2f}",
        ])
