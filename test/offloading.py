...
from utils.decoding import MSpecKV, Autoregressive
...
if __name__ == "__main__":
    ...
    print_config(draft, target, prefill, gen_len, gamma, top_k, top_p, temperature, file_path=None, method="MSpecKV (Offloading)", spec_args={'budget': args.budget, 'chunk_size': chunk_size}, dataset=args.dataset)
    ...
    # n_warmups = 3
    # input_ids = tokenized_prompts[0].to(target.device)[:,:prefill]
    # for i in tqdm(range(n_warmups), desc="MSpecKV Warmup"):
    #     MSpecKV(tokenizer, graph_engine, input_ids, gamma=gamma, max_len=gen_len, top_k=top_k, top_p=top_p, temperature=temperature, verbose=verbose, file_path=None, dataset=args.dataset, spec_args={'budget': args.budget, 'draft': args.draft, 'chunk_size': chunk_size, 'baseline': baseline_latency/1000})
    ...
    print(colored(f"[MSpecKV] average latency: {method_latency} ms", "red"))
