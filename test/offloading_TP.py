...
def parse_arguments():
    ...
    parser.add_argument('--compare', action='store_true', help='run baseline + TriForce + Quantized_TriForce')
...
def _init_llm_triforce(kv_quant: bool):
...
def _run_triforce(name: str, kv_quant: bool):
    gamma = int(args.gamma)
    llm = _init_llm_triforce(kv_quant=kv_quant)
    lats, accs = [], []
    for prompt in tqdm(tokenized_prompts, desc=name, disable=(local_rank != 0)):
        prompt = prompt[:, :prefill].to(llm.device)
        avg_tokens, lat = TriForce_Dist(tokenizer, llm, prompt, gamma=gamma, max_len=gen_len, top_k=-1, top_p=top_p, temperature=temperature, verbose=False, file_path=None, dataset=args.dataset)
...
if args.compare:
    b = _run_baseline()
    t, _ = _run_triforce("TriForce", kv_quant=False)
    q, _ = _run_triforce("Quantized_TriForce", kv_quant=True)
    if local_rank == 0:
        print(f"\nSpeedup vs AR: TriForce {b/t:.2f}x | Quantized_TriForce {b/q:.2f}x")
