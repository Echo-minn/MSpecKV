...
@torch.inference_mode()
def Quantized_TriForce(tokenizer, graph_engine, input_ids, gamma=4, max_len=256, top_k=-1, top_p=0.9, temperature=0.6, verbose=False, file_path=None, dataset=None, spec_args=None, kv_cache_quant=False, kv_cache_quant_bits=8, kv_fp16_tail=0, kv_resident_gpu=True, kv_resident_max_layers=16):
    # Quantization is implemented in the underlying engine/caches (e.g., offloading TP engines).
    # If the caller constructed a quantized engine, TriForce already uses it.
...
@torch.inference_mode()
def Quantized_TriForce_Dist(tokenizer, llm, input_ids, gamma=4, max_len=256, top_k=-1, top_p=0.9, temperature=0.6, verbose=False, file_path=None, dataset=None, spec_args=None, kv_cache_quant=True, kv_cache_quant_bits=8, kv_fp16_tail=0, kv_resident_gpu=True, kv_resident_max_layers=16):
    if kv_cache_quant:
        if not hasattr(llm, "kv_cache") or not hasattr(llm.kv_cache, "kv_quant_enabled"):
            raise ValueError("Quantized_TriForce_Dist expects an offloading TP llm with kv_cache_quant support.")
