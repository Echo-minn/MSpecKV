...
@torch.inference_mode()
def MSpecKV(tokenizer, graph_engine, input_ids, gamma=4, max_len=256, top_k=-1, top_p=0.9, temperature=0.6, verbose=False, file_path=None, dataset=None, spec_args=None):
...
@torch.inference_mode()
def Quantized_MSpecKV(tokenizer, graph_engine, input_ids, gamma=4, max_len=256, top_k=-1, top_p=0.9, temperature=0.6, verbose=False, file_path=None, dataset=None, spec_args=None, kv_cache_quant=False, kv_cache_quant_bits=8, kv_fp16_tail=0, kv_resident_gpu=True, kv_resident_max_layers=16):
    # Quantization is implemented in the underlying engine/caches (e.g., offloading TP engines).
    # If the caller constructed a quantized engine, MSpecKV already uses it.
    _ = (kv_cache_quant, kv_cache_quant_bits, kv_fp16_tail, kv_resident_gpu, kv_resident_max_layers)
    return MSpecKV(
        tokenizer=tokenizer,
...
@torch.inference_mode()
def VanillaSpecDec_Dist(tokenizer, llm, input_ids, gamma=4, max_len=256, top_k=-1, top_p=0.9, temperature=0.6, verbose=False, local_rank=0):
    """
    Vanilla Speculative Decoding without retrieval cache.
    Uses draft model for speculation but no retrieval cache for target model.
    """
    bsz, prefill_len = input_ids.size()
    llm.reset()
    
    # Regular prefill without retrieval cache
    llm.prefill(input_ids=input_ids[:,:-1])
    logits = llm.inference(input_ids=input_ids[:,-1:])
    next_token = sample_dist(norm_logits(logits[:,-1,:], temperature=temperature, top_k=top_k, top_p=top_p))
    ...
    while n < max_len:
        ...
        draft_n = 0
        while draft_n < gamma:
            speculation_prob = llm.draft_run(input_ids=verify_tokens[:,:draft_n+1], gamma_offset=draft_n)
            pred_token_idx = sample_dist(speculation_prob)
            draft_count += 1
            return_speculation_probs.append(speculation_prob)
            return_generated_ids.append(pred_token_idx.item())
            verify_tokens[:, draft_n+1] = pred_token_idx
            draft_n += 1
        
        generated_ids = return_generated_ids[1:]
        speculation_probs = return_speculation_probs
        gamma2 = len(generated_ids)
    ...
    acceptance_rate = accepted_count / draft_count if draft_count > 0 else 0
    avg_tokens = accepted_count / draft_count * gamma if draft_count > 0 else 0
    
    return avg_tokens, (time2 - time1)


@torch.inference_mode()
def MSpecKV_Dist(tokenizer, llm, input_ids, gamma=4, max_len=256, top_k=-1, top_p=0.9, temperature=0.6, verbose=False, file_path=None, dataset=None, spec_args=None):
...
@torch.inference_mode()
def Quantized_MSpecKV_Dist(tokenizer, llm, input_ids, gamma=4, max_len=256, top_k=-1, top_p=0.9, temperature=0.6, verbose=False, file_path=None, dataset=None, spec_args=None, kv_cache_quant=True, kv_cache_quant_bits=8, kv_fp16_tail=0, kv_resident_gpu=True, kv_resident_max_layers=16):
    if kv_cache_quant:
        if not hasattr(llm, "kv_cache") or not hasattr(llm.kv_cache, "kv_quant_enabled"):
            raise ValueError("Quantized_MSpecKV_Dist expects an offloading TP llm with kv_cache_quant support.")
        if not llm.kv_cache.kv_quant_enabled:
            raise ValueError("llm.kv_cache.kv_quant_enabled is False; construct llm with kv_cache_quant=True.")
        if getattr(llm.kv_cache, "kv_quant_bits", 8) != int(kv_cache_quant_bits):
            raise ValueError(f"kv_cache_quant_bits mismatch: llm has {getattr(llm.kv_cache, 'kv_quant_bits', None)} but got {kv_cache_quant_bits}.")
        if bool(getattr(llm.kv_cache, "kv_resident_gpu", False)) != bool(kv_resident_gpu):
            raise ValueError("kv_resident_gpu mismatch; construct llm with kv_resident_gpu matching this flag.")
        if int(getattr(llm.kv_cache, "kv_resident_max_layers", 0)) != int(kv_resident_max_layers):
            raise ValueError("kv_resident_max_layers mismatch; construct llm with kv_resident_max_layers matching this flag.")
        if int(getattr(llm.kv_cache, "kv_fp16_tail", 0)) != int(kv_fp16_tail):
            raise ValueError("kv_fp16_tail mismatch; construct llm with kv_fp16_tail matching this flag.")
    ...
    while n < max_len:
        ...
        pred_token_idx = next_token
        verify_tokens, speculation_probs, _acc_mid = Middle_Spec_Dist(pred_token_idx, llm, gamma, False, tokenizer)
        generated_ids = verify_tokens[1:]
        draft_count += len(speculation_probs)
        gamma2 = len(generated_ids)
    ...
    time2 = time.time()
    denom = max(draft_count, 1)
    acceptance_rate = accepted_count / denom
    avg_tokens = acceptance_rate * gamma

    if file_path is not None:
        header = "target,acceptance_rate,token/s,avg_tokens,prefill,gen_len,dataset,latency\n"
        entry = f"{llm.config._name_or_path},{acceptance_rate},{max(n,1)/(time2-time1)},{avg_tokens},{input_ids.shape[1]},{n},{dataset},{(time2-time1)/max(n,1)}\n"
        if spec_args is not None:
            for k, v in spec_args.items():
                header = header.replace("\n", f",{k}\n")
                entry = entry.replace("\n", f",{v}\n")
    ...
    return avg_tokens, (time2 - time1) / max(n, 1)
